...
func display_difficulty():
	...
	if spell_data.difficulty_modifier != 0:
		$DifficultyContainer/Difficulty.text += " (" + ("+" if spell_data.difficulty_modifier > 0 else "") + str(spell_data.difficulty_modifier) + " extras)"
...
func _on_reduce_difficulty():
	if spell_data.current_difficulty > 1:
		spell_data.difficulty_modifier -= 1
	display_difficulty()
